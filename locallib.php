...
/**
 * @Solutto
 * Get custom certificate:  <userFullName>_<userID>_<coursename>_<certificateName>
 * @param stdClass $user
 * @param stdClass $course
 * @param stdClass $certificate
 ...
 * 
 */
function custom_certificate_filename($user, $course, $certificate) {
    $custom_filename = fullname($user). '_' .$user->idnumber . '_' . $course->fullname . '_' . $certificate->name;
    $custom_filename = core_text::entities_to_utf8($custom_filename);
    $custom_filename = strip_tags($custom_filename);
    $custom_filename = rtrim($custom_filename, '.');
    $custom_filename = clean_filename($custom_filename);
...
function certificate_print_user_files($certificate, $userid, $contextid) {
    ...
    $fs = get_file_storage();
    # @Solutto
    # Modify the certificate show in the report.php to <userFullName>_<rut>_<coursename>_<certificateName>
    ...
    foreach ($files as $file) {
        ...
        $output .= $fileicon . '&nbsp;' . '<a href="'.$link.'" >'.s($custom_filename).'</a>'; #@Solutto Change in this section only the name show, not change the $filename
...
function certificate_get_certificate_filename($certificate, $cm, $course) {
    ...
    # @Solutto
    # Modify the certificate filename to <userFullName>_<rut>_<coursename>_<certificateName>
    global $USER;
    $filename = custom_certificate_filename($USER, $course, $certificate);
